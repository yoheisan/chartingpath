...
export const StrategyExecution = ({ strategies, onRefresh }: StrategyExecutionProps) => {
  ...
  const simulateStrategyExecution = async () => {
    ...
    if (Math.random() < 0.1) {
      ...
      const symbols = Object.keys(MOCK_MARKET_DATA);
      const symbol = symbols[Math.floor(Math.random() * symbols.length)];
      const marketData = MOCK_MARKET_DATA[symbol as keyof typeof MOCK_MARKET_DATA];
      ...
      try {
        ...
        const { error: tradeError } = await supabase
          ...
          .insert({
            user_id: user.id,
            portfolio_id: user.id, // Simplified for demo
            symbol,
        ...
        toast({
          ...
          description: `${strategy.name}: ${signalType.toUpperCase()} ${quantity} ${symbol} at ${marketData.price}`,
