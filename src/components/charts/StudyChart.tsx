...
const StudyChart = memo(({ 
  ...
  useEffect(() => {
    ...
    if (hasVolume) {
      ...
      // Build set of significant volume bar timestamps from historical patterns
      const sigVolSet = new Set<number>();
      if (historicalPatterns) {
        for (const p of historicalPatterns) {
          if (p.significantVolumeBars) {
            for (const ts of p.significantVolumeBars) {
    ...
    if (historicalPatterns && historicalPatterns.length > 0 && patternToggles.showPatterns) {
      ...
      if (currentPattern && currentPattern.pivots && currentPattern.pivots.length > 0 && patternToggles.showLabels) {
        ...
        currentPattern.pivots.forEach((pivot) => {
          // Skip "Entry" pivot — entry is rendered as a native marker below
