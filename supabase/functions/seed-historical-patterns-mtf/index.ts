...
async function runHistoricalBacktest(
  ...
  for (let i = lookback; i < bars.length - maxBarsInTrade; i++) {
    ...
    const atrFloor = atr * 1.0;
    function getP(label: string): number | null {
      return detectionResult.pivots.find(p => p.label === label)?.price ?? null;
    }
    let stopLoss: number;
    ...
    if (p === 'double-top' || p === 'triple-top') {
      const tops = ['Top 1','Top 2','Top 3'].map(l => getP(l)).filter(Boolean) as number[];
      const nk = getP('Neckline');
      if (tops.length >= 2 && nk) { const pk = Math.max(...tops); sl = pk * 1.005; tp = nk - (pk - nk); }
    } else if (p === 'double-bottom' || p === 'triple-bottom') {
      const bots = ['Bottom 1','Bottom 2','Bottom 3'].map(l => getP(l)).filter(Boolean) as number[];
      const nk = getP('Neckline');
      if (bots.length >= 2 && nk) { const tr = Math.min(...bots); sl = tr * 0.995; tp = nk + (nk - tr); }
    } else if (p === 'head-and-shoulders') {
      const rs = getP('Right Shoulder'); const hd = getP('Head'); const nk = getP('Neckline');
      if (rs && hd && nk) { sl = rs * 1.005; tp = nk - (hd - nk); }
    } else if (p === 'inverse-head-and-shoulders') {
      const rs = getP('Right Shoulder'); const hd = getP('Head'); const nk = getP('Neckline');
      if (rs && hd && nk) { sl = rs * 0.995; tp = nk + (nk - hd); }
    } else if (p === 'bull-flag') {
      const fh = getP('Flag High'); const ps = getP('Pole Start'); const pe = getP('Pole End');
      if (fh && ps && pe) { sl = fh * 0.995; tp = entryPrice + (pe - ps); }
    } else if (p === 'bear-flag') {
      const fl = getP('Flag Low'); const ps = getP('Pole Start'); const pe = getP('Pole End');
      if (fl && ps && pe) { sl = fl * 1.005; tp = entryPrice - (ps - pe); }
    } else if (p === 'ascending-triangle') {
      const res = getP('Resistance'); const sup = getP('Rising Support');
      if (res && sup) { sl = sup * 0.995; tp = entryPrice + (res - sup); }
    } else if (p === 'descending-triangle') {
      const sup = getP('Support'); const res = getP('Falling Resistance');
      if (sup && res) { sl = res * 1.005; tp = entryPrice - (res - sup); }
    } else if (p === 'rising-wedge') {
      const ue = getP('Upper Trend End'); const ls = getP('Lower Trend Start');
      if (ue && ls) { sl = ue * 1.005; tp = entryPrice - (ue - ls); }
    } else if (p === 'falling-wedge') {
      const us = getP('Upper Trend Start'); const le = getP('Lower Trend End');
      if (us && le) { sl = le * 0.995; tp = entryPrice + (us - le); }
    } else if (p === 'cup-and-handle') {
      const lr = getP('Left Rim'); const cb = getP('Cup Bottom'); const hl = getP('Handle') ?? getP('Right Rim');
      if (lr && cb && hl) { sl = hl * 0.995; tp = entryPrice + (lr - cb); }
    } else if (p === 'inverse-cup-and-handle') {
      const ct = getP('Cup Top'); const rr = getP('Right Rim'); const hl = getP('Handle');
      if (ct && rr && hl) { sl = hl * 1.005; tp = entryPrice - (ct - rr); }
    } else if (p === 'symmetrical-triangle') {
    ...
    const qualityInput: PatternQualityScorerInput = {
      ...
      takeProfit,
      atr,
      repeatabilityProof: cachedRepeatabilityProof,
      historicalPerformance: cachedHistoricalPerformance,
      timeframe,
