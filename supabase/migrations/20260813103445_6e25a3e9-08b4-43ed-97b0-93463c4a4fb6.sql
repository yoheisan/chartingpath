  ...
  LOOP
    ...
    BEGIN
      ...
      WHEN 'prices_fresh_per_asset_class' THEN
        ...
        FROM (
          SELECT i.asset_type,
                 max((SELECT max(hp.updated_at) FROM public.historical_prices hp
                       WHERE hp.symbol = i.symbol
                         AND hp.updated_at > now() - interval '14 days')) AS last_seen
          FROM public.instruments i
          WHERE i.is_active
          GROUP BY i.asset_type
        ) x
        WHERE x.last_seen IS NULL OR x.last_seen < now() - interval '48 hours';
        v_passed := coalesce(array_length(v_arr,1),0) = 0;
        v_obs := CASE WHEN v_passed THEN 'all asset classes priced'
                      ELSE 'stale: ' || array_to_string(v_arr, ', ') END;
      ...
      WHEN 'scanner_coverage' THEN
        SELECT array_agg(x.asset_type || ' ' || x.fresh || '/' || x.total ORDER BY x.asset_type),
               jsonb_object_agg(x.asset_type, jsonb_build_object('fresh', x.fresh, 'total', x.total))
          INTO v_arr, v_detail
        FROM (
          SELECT i.asset_type,
                 count(*) AS total,
                 count(*) FILTER (WHERE EXISTS (
                   SELECT 1 FROM public.historical_prices hp
                   WHERE hp.symbol = i.symbol AND hp.updated_at > now() - interval '48 hours'
                 )) AS fresh
          FROM public.instruments i
          WHERE i.is_active
          GROUP BY i.asset_type
      ...
      WHEN 'cron_jobs_producing_output' THEN
        ...
        LOOP
          EXECUTE format(
            'SELECT count(*) FROM public.%I WHERE %I > now() - interval ''%s hours''',
    ...
    EXCEPTION WHEN OTHERS THEN
      -- A check that errors is a FAILURE, never a pass. A monitoring
      -- system that silently stops monitoring is the failure mode we
      -- are fixing.
      v_passed := false;
...
REVOKE ALL ON FUNCTION public.run_data_health_checks(text) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.run_data_health_checks(text) TO service_role;