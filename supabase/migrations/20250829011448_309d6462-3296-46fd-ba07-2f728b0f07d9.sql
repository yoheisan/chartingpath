...
UPDATE plan_pricing SET 
  ...
  END,
  features = CASE
    WHEN plan = 'free' THEN '{"chart_patterns": 5, "backtesting": "demo_only", "algo_builder": "sandbox_only", "community_sharing": false}'::jsonb
  ...
  END,
  updated_at = now()
WHERE plan IN ('free', 'starter', 'pro', 'pro_plus', 'elite');

-- Insert pro_plus pricing if it doesn't exist
INSERT INTO plan_pricing (plan, monthly_price_cents, yearly_price_cents, max_alerts, features, is_active)
VALUES 
